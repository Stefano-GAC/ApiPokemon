let limit = 9; // ✅ ahora se cargan 9 Pokémon por página
let currentPage = 1;
let tipoActual = null;
let tipoPagina = 1;
let tipoLista = [];
...
function cargarBusqueda(nombre) {
    ...
    $.get(`https://pokeapi.co/api/v2/pokemon/${query}`)
        ...
        .catch(() => {
            $(".gallery .row").html('<p class="text-center text-danger">Pokémon no encontrado</p>');
        });
}
...
function mostrarDetalle(nombre) {
    $.get(`https://pokeapi.co/api/v2/pokemon/${nombre}`)
        .then((data) => {
            ...
            const html = `
                <div class="pokemon-card">
                    <div class="left-info">
                        <img src="${data.sprites.other['official-artwork'].front_default || data.sprites.front_default}" alt="${data.name}" class="img-fluid mb-3">
        ...
        .catch(() => {
            $("#modalContent").html('<p class="text-center text-danger">No se pudieron cargar los detalles</p>');
            const modal = new bootstrap.Modal(document.getElementById("pokemonModal"));
            modal.show();
        });
}
